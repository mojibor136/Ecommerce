    ...
    <div class="w-full mb-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 border-b rounded-md mb-4">
            <div class="flex flex-col gap-2 w-full md:w-2/3">
                ...
                <p class="text-sm text-gray-500 ml-1">Manage all product reviews efficiently</p>
            </div>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-indigo-600 text-white text-sm font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left whitespace-nowrap">Product</th>
                        <th class="px-4 py-3 text-left whitespace-nowrap">Title</th>
                        <th class="px-4 py-3 text-center whitespace-nowrap">Rating</th>
                        <th class="px-4 py-3 text-left whitespace-nowrap">Review</th>
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-200">
                    @foreach ($reviews as $index => $review)
                        <tr class="hover:bg-gray-50 transition-colors cursor-pointer">
                            <td class="px-4 py-3 text-center whitespace-nowrap">{{ $reviews->firstItem() + $index }}</td>

                            <td class="px-4 py-3 text-left flex items-center gap-2 w-56">
                                <img src="{{ asset('uploads/products/' . ($review->product->images->first()->image ?? 'default.png')) }}"
                                    alt="{{ $review->product->name ?? 'N/A' }}"
                                    class="w-10 h-10 object-cover rounded-full mt-1">
                                <span class="line-clamp-2 break-words text-sm">
                                    {{ $review->product->name ?? 'N/A' }}
                                </span>
                            </td>

                            <td class="px-4 py-3 text-left whitespace-nowrap">
                                {{ Str::limit($review->title, 14) }}
                            </td>
                    ...
                    @if ($reviews->isEmpty())
                        <tr>
                            <td colspan="7" class="py-4 px-3 text-center text-gray-400">No reviews found.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
