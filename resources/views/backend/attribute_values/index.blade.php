    ...
    <div class="w-full mb-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 border-b rounded-md mb-4">
            <div class="flex flex-col gap-2 w-full md:w-2/3">
                <h1 class="text-xl font-bold text-gray-800">Attribute Values Management</h1>
                <p class="text-sm text-gray-500 ml-1">Manage all attribute values efficiently</p>
            </div>
            <div class="flex flex-row gap-2 mt-3 md:mt-0 w-full md:w-auto items-start sm:items-center">
                <a href="{{ route('admin.attribute_values.create') }}"
                    ...
                    <i class="ri-add-line text-lg"></i> Add Attribute Value
                </a>
            </div>
        </div>

        <!-- Search & Filter -->
        <form method="GET" action="{{ route('admin.attribute_values.index') }}"
            ...
            <div class="flex flex-col sm:flex-row w-full sm:w-2/3 gap-2">
                ...
                <div class="relative w-full sm:w-1/2">
                    <select name="attribute_id"
                        class="w-full rounded-md bg-white text-gray-900 border border-gray-300 pl-3 pr-8 py-2.5
                        text-sm sm:text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200">
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-indigo-600 text-white text-sm font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left whitespace-nowrap">Attribute</th>
                        <th class="px-4 py-3 text-left whitespace-nowrap">Value</th>
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-200">
                    @foreach ($values as $index => $value)
                        <tr class="hover:bg-gray-50 transition-colors cursor-pointer">
                            <td class="px-4 py-3 text-center whitespace-nowrap">{{ $values->firstItem() + $index }}</td>
                            <td class="px-4 py-3 text-left whitespace-nowrap">{{ $value->attribute->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-left whitespace-nowrap">{{ $value->value }}</td>
                            ...
                            <td class="px-4 py-3 text-right whitespace-nowrap">
                                <div class="flex justify-end items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.attribute_values.destroy', $value->id) }}" method="POST"
                    ...
                    @if ($values->isEmpty())
                        <tr>
                            <td colspan="6" class="py-4 px-3 text-center text-gray-400">No attribute values found.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
