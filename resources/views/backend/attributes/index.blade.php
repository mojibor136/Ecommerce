    ...
    <div class="w-full mb-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 border-b rounded-md mb-4">
            <div class="flex flex-col gap-2 w-full md:w-2/3">
                <h1 class="text-xl font-bold text-gray-800">Attribute Management</h1>
                <p class="text-sm text-gray-500 ml-1">Manage your attributes efficiently</p>
            </div>
            <div class="flex flex-row gap-2 mt-3 md:mt-0 w-full md:w-auto items-start sm:items-center">
                <a href="{{ route('admin.attributes.create') }}"
                    ...
                    <i class="ri-add-line text-lg"></i> Add Attribute
                </a>
            </div>
        </div>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-[{{ $theme->theme_bg }}] text-[{{ $theme->theme_text }}]  text-sm font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left whitespace-nowrap">Attribute Name</th>
                        <th class="px-4 py-3 text-left whitespace-nowrap">Slug</th>
                        <th class="px-4 py-3 text-center whitespace-nowrap">Values Count</th>
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-200">
                    @foreach ($attributes as $index => $attribute)
                        <tr class="hover:bg-gray-50 transition-colors cursor-pointer">
                            ...
                            <td class="px-4 py-3 text-left whitespace-nowrap">{{ $attribute->name }}</td>
                            <td class="px-4 py-3 text-left whitespace-nowrap">{{ $attribute->slug }}</td>
                    ...
                    @if ($attributes->isEmpty())
                        <tr>
                            <td colspan="5" class="py-4 px-3 text-center text-gray-400">No attributes found.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
